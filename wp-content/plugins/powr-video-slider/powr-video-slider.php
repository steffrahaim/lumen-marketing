    ...
    */
    ...
    class Powr_Video_Slider extends WP_Widget{
      ...
      public function form( $instance ){
        ...
        ?>
        <p>
          To edit, visit your live webpage and click the gears icon on your Video Slider.
        </p>
        <p>
    ...
    function powr_video_slider_shortcode( $atts ){
      if(isset($atts['id'])){
        $id = $atts['id'];
      	return "<div class='powr-video-slider' id='$id'></div>";
      }else if(isset($atts['label'])){
        $label = $atts['label'];
		    return "<div class='powr-video-slider' label='$label'></div>";
      }else{
      	"<div class='powr-video-slider'></div>";
      }
    }
    ...
    //ADD MENUS
    add_action( 'admin_menu', 'powr_video_slider_menu' );
    function powr_video_slider_menu() {
      add_menu_page( 'POWr Video Slider', 'POWr Video Slider', 'manage_options', 'powr-video-slider-settings', powr_video_slider_options, plugins_url('/powr-icon.png',__FILE__));
    }
    function powr_video_slider_options() {
      if ( !current_user_can( 'manage_options' ) )  {
        wp_die( __( 'You do not have sufficient permissions to access this page.' ) );
      }
      echo '<iframe id="powr-video-slider" src="https://www.powr.io/wp-create/video-slider" frameborder="0" width="100%" height="600px" style="position:absolute; top:0; left:0;z-index: 100;"></iframe>';
      echo '<script>';
      echo 'var ht = window.innerHeight; if(document.getElementById("wpadminbar")){ht -= document.getElementById("wpadminbar").offsetHeight;}';
      echo 'var iframe = document.getElementById("powr-video-slider"); iframe.style.height = ht+"px"; iframe.height = ht;';
      echo '</script>';
    }
    //Redirecting to landing page when plugin is activated
    register_activation_hook(__FILE__, 'powr_video_slider_plugin_activate');
      add_action('admin_init', 'powr_video_slider_plugin_redirect');

      function powr_video_slider_plugin_activate() {
      add_option('powr_video_slider_plugin_do_activation_redirect', true);
      }

      function powr_video_slider_plugin_redirect() {
      if (get_option('powr_video_slider_plugin_do_activation_redirect', false)) {
          delete_option('powr_video_slider_plugin_do_activation_redirect');
          if(!isset($_GET['activate-multi']))
          {
            wp_redirect( get_admin_url().'?platform=wordpress&page=powr-video-slider-settings' );
          }
       }
      }
