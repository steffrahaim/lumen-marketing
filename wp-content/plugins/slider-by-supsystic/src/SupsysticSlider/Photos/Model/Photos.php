...
//        }
    ...
    {
        // An array of URL components generated by parse_url() function
        $arUrl = parse_url($url);
        // Return value. By default, we will consider incorrect URL
        $ret = null;

        // If protocol is not specified - the protocol is invalid for URL
        if (!array_key_exists("scheme", $arUrl) || !in_array($arUrl["scheme"], array("http", "https")))
            // Set default protocol - http
            $arUrl["scheme"] = "http";

        // If parse_url() function is determine host
        if (array_key_exists("host", $arUrl) && !empty($arUrl["host"])) {
            // Constructing the final value of URL
            $ret = sprintf("%s://%s%s", $arUrl["scheme"], $arUrl["host"], $arUrl["path"]);
        }
        // If host is not defined - checking $arUrl["path"] at the URL pattern matching
        else if (preg_match("/^\w+\.[\w\.]+(\/.*)?$/", $arUrl["path"])) {
            // Constructing the final value of URL
            $ret = sprintf("%s://%s", $arUrl["scheme"], $arUrl["path"]);
        }

        // If the URL is valid and we have some string query parameters
        if ($ret && !empty($arUrl["query"]))
            $ret .= sprintf("?%s", $arUrl["query"]);

        // If the URL is valid and we have some anchor
        if ($ret && !empty($arUrl["fragment"]))
            $ret .= sprintf("#%s", $arUrl["fragment"]);
