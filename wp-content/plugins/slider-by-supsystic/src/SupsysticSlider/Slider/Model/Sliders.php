    ...
    implements Rsc_Environment_AwareInterface
{
    ...
    public function getAll()
    {
        ...
        if (is_array($sliders) && count($sliders) > 0) {
			foreach($sliders as $key => $slider) {
				$sliders[$key]->title = $this->specialCharsDecode($sliders[$key]->title);
			}
            if($this->environment) {
    ...
    public function getBy($field, $value, $mode = self::MODE_COLLECTION)
    {
		...
		if($result && $result->title) {
			$result->title = $this->specialCharsDecode($result->title);
		}
	...
	/**
	 * Decoding special chars in the text data selected from db.
	 *
	 * @return string $text Decoded text
	 */
	public function specialCharsDecode($text)
	{
		if (is_array($text)) {
			return array_map(array($this, 'enter'), $text);
		}
