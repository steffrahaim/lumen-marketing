...
(function ($, _wp, _app, _debug, undefined) {
    ...
    Controller.prototype.saveSettings = (function($sidebar, $slider, e) {
        ...
        function rgbaToHexWithOpacity(rgb) {
            if (rgb.substr(0, 1) == '#') {
                return {
                    hex: rgb,
                    opacity: 1
        ...
        $.post(_wp.ajax.settings.url, {
            ...
            $.each($sidebar.find(':input'), function(index, el) {
                $input = $(this);
                prop = $input.data('prop');
                if ($input.data('selector') == '.bx-caption') {
                    value = $input.val();
                    ...
                    if (prop == 'background-color') {
                        color = rgbaToHexWithOpacity(value);
                        $('[name="caption[background-color-hex]"]').val(color.hex).trigger('change');
                        $('[name="caption[background-opacity]"]').val(color.opacity).trigger('change');
                    }

                    $('[name="caption[' + prop + ']"]').val(value).trigger('change');
                }
            });
            ...
            open: function () {
                ...
                $.post(_wp.ajax.settings.url, {
                    ...
                    $slider.append('<div class="bx-viewport-button button" style="display: inline-block; margin: 10px;">Viewport</div>' +
                                    ...
                                    '<div class="bx-prev-button button" style="display: inline-block; margin: 10px;">Buttons</div>').css('visibility', 'visible');
